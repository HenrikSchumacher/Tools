...
namespace Tools
{
    ...
    force_inline void copy_buffer( const S * restrict const from, T * restrict const to, const size_t n )
    {
    ...
    force_inline void copy_buffer( const S * restrict const from, T * restrict const to )
    {
        if constexpr ( std::is_same_v<T,S> )
        {
            std::copy( &from[0], &from[n], &to[0] );
