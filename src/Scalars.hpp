...
//    using Complex128 = std::complex<Real128>;
    ...
    namespace Scalar
    {
        ...
        template<typename T>
        [[nodiscard]] force_inline constexpr Flag ToFlag( const T x )
        {
            if( x == T(0) )
            {
                return Flag::Zero;
        ...
        template<typename T>
        [[nodiscard]] force_inline std::string ToFlagString( const T x )
        {
            if( x == T(0) )
            {
    ...
    [[nodiscard]] force_inline std::string ToString( const Scalar::Flag flag )
    {
        switch( flag )
        {
...
#define ASSERT_COMPLEX(C) static_assert( Scalar::ComplexQ<C>, "Template parameter " #C " must be a complex-valued type." );
    ...
    namespace Scalar
    {
        ...
        [[nodiscard]] force_inline constexpr bool OpReturnRealQ()
        {
